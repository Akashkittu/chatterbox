...
npm install @clerk/nextjs- For Clerk's frontend + Next.js integration
Used in components like <SignIn />, useUser(), etc.

npm install convex @clerk/backend-For:
                                      Convex SDK (to write server-side queries/mutations)
                                      Clerk backend SDK (to authenticate Convex functions using Clerk sessions)
npx convex dev->.env.local created and this should be run whole time with npm run dev

create a clerk account and get ->NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY and CLERK_SECRET_KEY paste these two in .env
